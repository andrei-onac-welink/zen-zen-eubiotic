---
id: 4
uuid: "31ac9f6a-33b7-46d7-84d8-33d9e3390008"
title: 'Eubiotic Baby Picături - pentru cei mici'
linkTitle: 'Eubiotic Baby Picături'
description: 'Eubiotic Baby Picături oferă o formulă de bacterii benefice special concepută pentru cei mici, contribuind la sănătatea digestivă și echilibrul microbiotei.'
slug: '/eubiotic-baby-picaturi/'
weight: 4
ean: ''
featuredImage:
  url: '/images/products/eubiotic-baby-picaturi-stg.png'
  
resources:
  - name: image
    src: '/images/products/eubiotic-baby-picaturi-stg.png'
    title: ''
  - name: bb12
    src: '/images/products/extras/scb_bb-12_logo_blue_rgb.png'
    title: 'Bifidobacterium BB-12'
  - name: lgg
    src: '/images/products/extras/scb_lgg_logo_blue_rgb.png'
    title: 'Lactobacillus rhamnosus LGG'
  - name: baby
    src: '/images/products/extras/baby-white-bg.png'
    title: 'Bebeluș'
    
blocksFolder: 'products'
blocks:
  - component: 'hero'
    title: 'Eubiotic Baby Picături'
    productName: 'Eubiotic<sup>®</sup> Baby Picături'
    heading: 'Ce este și cum se utilizează'
    image: 'image'
    paragraphs:
      - 'Eubiotic Baby<sup>®</sup> picături conține culturi lactice vii selecționate din cele mai bine documentate științific tulpini de <strong>Lactobacillus rhamnosus LGG<sup>®</sup></strong> și <strong>Bifidobacterium BB-12<sup>®</sup></strong> pe populații reprezentate de sugari și copii mici.'
      - 'Eubiotic Baby<sup>®</sup> picături este recomandat în situațiile în care este necesară regenerarea florei intestinale, cum ar fi cazurile de diaree produsă de consumul de antibiotice și agenți patogeni: virusuri, bacterii, paraziți.'
    ctaLabel: 'Citește prospectul'
    ctaPage: 'product/prospect/prospect-eubiotic-baby-picaturi'
    
  - component: 'strains'
    items:
      - image: 'bb12'
        alt: 'Logo Bifidobacterium BB-12'
        introduction: 'Află ce beneficii aduce <strong>Bifidobacterium, BB-12<sup>®</sup></strong>, chiar de la producător'
        href: 'https://www.chr-hansen.com/'
      - image: 'lgg'
        alt: 'Logo Lactobacillus rhamnosus LGG'
        introduction: 'Află ce beneficii aduce <strong>Lactobacillus rhamnosus, LGG<sup>®</sup></strong>, pentru copilul tău chiar de la producător'
        href: 'https://www.chr-hansen.com/'
        
  - component: 'details'
    ariaLabel: 'Beneficiile tulpinilor Eubiotic Baby Picături'
    sections:
      - title: 'Lactobacillus rhamnosus (LGG)'
        paragraphs:
          - 'Este cea mai documentată tulpină bacteriană benefică din lume, peste 1000 de publicații științifice și 300 de studii clinice demonstrându-i beneficiile în regenerarea florei intestinale. <em>Lactobacillus rhamnosus</em> GG<sup>®</sup> (LGG) a demonstrat beneficii în:'
        groups:
          - title: 'Sănătatea tractului digestiv'
            items:
              - 'Reducerea riscului apariției diareii în cazul administrării de antibiotice'
              - 'Scăderea riscului apariției diareilor nosocomiale'
              - 'Reducerea frecvenței colicilor'
              - 'Diminuarea riscului diareei acute'
          - title: 'Imunitate'
            items:
              - 'Scăderea riscului infecțiilor respiratorii'
      - title: 'Bifidobacterium BB-12'
        paragraphs:
          - 'Bifidobacterium este principala tulpină bacteriană benefică care populează tubul digestiv la copii mici și sugari, ea regăsindu-se în laptele matern. Studiile pe populația pediatrică demonstrează beneficiile acestei tulpini în:'
        groups:
          - title: 'Sănătatea tractului digestiv'
            items:
              - 'Reducerea riscului apariției diareii cauzată de consumul de antibiotice'
              - 'Scăderea riscului apariției colicilor și a iritabilității de cauză digestivă'
              - 'Reducerea duratei diareii de orice etiologie'
              - 'Menținerea sănătății intestinale după înțărcare'
              - 'Diminuarea riscului apariției infecției cu rotavirus'
          - title: 'Imunitate'
            items:
              - 'Creșterea răspunsului imun la copiii născuți prin cezariană'
              - 'Reducerea riscului de infecții respiratorii'
              - 'Reducerea riscului alergiilor'
              
  - component: 'usage'
    image: 'baby'
    overlayVariant: 'primary'
    title: 'Cui se adresează Eubiotic Baby Picături?'
    introduction: 'Eubiotic Baby picături este recomandat în situațiile în care:'
    sections:
      - items:
          - 'Din prima zi de viață'
          - 'Este necesară regenerarea florei intestinale, cum ar fi cazurile de diaree cauzată de folosirea de antibiotice, virusuri, bacterii sau paraziți'
          - 'Pentru ameliorarea colicilor'
      - title: 'Mod de prezentare'
        items:
          - 'Sticluță cu 8 g lichid.'
      - title: 'Utilizare'
        paragraphs:
          - 'Este necesar să se administreze 6 picături de Eubiotic Baby picături zilnic, în doză unică, pe toată durata manifestării simptomelor. O doză conține 1 miliard de bacterii lactice vii.'
          - 'Eubiotic Baby picături se poate administra fie ca atare, într-o linguriță, fie împreună cu alimente sau lichide.'
          - 'Produsul nu trebuie dizolvat în lichide fierbinți pentru a nu afecta culturile lactice vii. Eubiotic Baby picături nu schimbă gustul alimentelor sau băuturilor cu care sunt amestecate.'
          - 'În cazul în care produsul se administrează pentru compensarea efectelor adverse ale antibioticelor, se recomandă administrarea Eubiotic Baby picături la un interval de 3 ore după administrarea acestora și încă o săptămână după terminarea tratamentului cu antibiotic.'
          - 'A se agita bine înainte de utilizare. Produsul nu se refrigerează.'
          
  - component: 'references'
    title: 'Referințe:'
    items:
      - 'Vanderhoof JA, Whitney DB, Antonson DL, et al. Lactobacillus GG in the prevention of antibiotic-associated diarrhea in children. J Pediatr. 1999;135:564-8.'
      - 'Arvola T, Laiho K, Torkkeli S, et al. Prophylactic Lactobacillus GG reduces antibiotic-associated diarrhea in children with respiratory infections: a randomized study. Pediatrics. 1999;104:e64.'
      - 'Saavedra JM, Abi-Hanna A, Moore N & Yolken RH. Long-term consumption of infant formulas containing live probiotic bacteria: tolerance and safety. Am J Clin Nutr 2004;79:261-267.'
---

Eubiotic Baby® picături conţine culturi lactice vii selecţionate din cele mai bine documentate ştiinţific tulpini de Lactobacillus rhamnosus LGG® şi Bifidobacterium BB-12® pe populaţii reprezentate de sugari şi copii mici.

Eubiotic Baby® picături este recomandat în situaţiile în care este necesară regenerarea florei intestinale, cum ar fi cazurile de diaree produsă de consumul de antibiotice și agenți patogeni: virusuri, bacterii, paraziți.

Produsul este destinat copiilor încă din prima zi de viață, precum și femeilor însărcinate și a celor care alăptează.
